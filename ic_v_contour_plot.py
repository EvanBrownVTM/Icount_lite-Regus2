...
For Vicki CV-equipped machines
Features
   		- 
...
Updates
        - SideCam product zone sent to matching function
		- runs on video instead of directly from camera
'''
...
from utils.display import open_window, set_display, display_fps
from utils.visualization_ic import BBoxVisualization
from utils.tracker import AVT
from utils.front_cam_solver_contour_plot import FrontCam
from utils.side_cam_solver_contour_plot import SideCam
from  utils.utils import descale_contour
...
logging.basicConfig(filename='/home/evan/Desktop/logs/Icount.log', level=logging.DEBUG, format='%(asctime)-8s %(levelname)-8s %(message)s')
...
cls_dict = {0: 'Vickies', 1: 'Coke', 2: 'Gatorade', 3: 'Sun_chips', 4: 'Doritos', 5: 'Lays'}
save_size = 200
display_mode = True
pika_flag = True

cam0_contour_logging = False
cam1_contour_logging = False
cam2_contour_logging = True
contours_path_0 = 'utils/cam0_zones_action.npz'
contours_path_1 = 'utils/cam1_zones_action.npz'
contours_path_2 = 'utils/cam2_zones_action.npz'
contours0 = np.load(contours_path_0, allow_pickle=True)
contours1 = np.load(contours_path_1, allow_pickle=True)
contours2 = np.load(contours_path_2, allow_pickle=True)
...
def trt_detect(frame, trt_yolo, conf_th, vis):
	if frame is not None:
		...
		if display_mode:
			frame = vis.draw_bboxes(np.uint8(frame), boxes, confs, clss)
...
def infer_engine(timestr, frame0, frame1, frame2, frame_cnt0, frame_cnt1, frame_cnt2, cv_activities_cam0, cv_activities_cam1, cv_activities_cam2, cv_pick_cam0, cv_ret_cam0, cv_pick_cam1, cv_ret_cam1, cv_pick_cam2, cv_ret_cam2):
	...
	file2info = {}
	file2info['bboxes'] = np.asarray(np.asarray(new_boxes0, dtype=np.int32) / 416 * 200, dtype = np.int32).tolist()
	...
	file2info1 = {}
	file2info1['bboxes'] = np.asarray(np.asarray(new_boxes1, dtype=np.int32) / 416 * 200, dtype = np.int32).tolist()
	...
	file2info2 = {}
	file2info2['bboxes'] = np.asarray(np.asarray(new_boxes2, dtype=np.int32) / 416 * 200, dtype = np.int32).tolist()
	...
	if id2active_zone0 and any(id2active_zone0.values()): 
		print('id2active_zone0', id2active_zone0)
	if id2active_zone1 and any(id2active_zone1.values()): 
		print('id2active_zone1', id2active_zone1)
	if id2active_zone2 and any(id2active_zone2.values()): 
		print('id2active_zone2', id2active_zone2)
	...
	#TODO: add additional item to cv_pick or cv_ret dicts with row each product came from
	#cam_solver should have ._tracks which has  ObjectID which corresponds to id2active_zone id

	#TODO: use cv_activities_cam<#> and cv_pick<#> and cv_ret<#> as input to function combining zone and 
	
	cart = merge_cart(cam0_solver, cam1_solver, cam2_solver)
	
	return det_frame0, det_frame1, det_frame2, cart#, id2active_zone0, id2active_zone1, id2active_zone2
...
def match(set1, set2, thresh=None):
    '''
        e.g.
            set1: [{'class_id':1, 'action':'PICK', 'timestamp': '2022-07-25:11:15:50', 'active_zone': upper_shelves}, {'class_id':1, 'action':'RETURN', 'timestamp': '2022-07-25:11:15:57', 'active_zone': top_shelf}]
            set1: [{'class_id':1, 'action':'RETURN', 'timestamp': '2022-07-25:11:16:00', 'active_zone': lower_shelves}]
    '''
    not_matched1 = []
    not_matched2 = []
...
def process_actions(cv_act_cam0, cv_act_cam2, timeout = 3, m_flag = False):
	...
	if not m_flag:
		for ind in m1_act:
			time1 = datetime.strptime(cv_act_cam0[ind][2], '%Y-%m-%d:%H:%M:%S')
	...
	if not m_flag:
		for ind in m2_act:
			time2 = datetime.strptime(cv_act_cam2[ind][2], '%Y-%m-%d:%H:%M:%S')
			curr_time = datetime.strptime(time.strftime(timestamp_format), timestamp_format)
...
def fuse_cam12_activities(cv_pick_cam1, cv_ret_cam1, cv_pick_cam2, cv_ret_cam2, cv_activities_fused):
	...
	if len(act_picks12) > 0:
		for act_pick in act_picks12:
			...
			print('                                           {} {} @ {}'.format(act_pick[1], act_pick[0], act_pick[2]))
	...
	if len(act_returns12) > 0:
		for act_return in act_returns12:
			...
			print('                                           {} {} @ {}'.format(act_return[1], act_return[0], act_return[2]))
...
cam0_solver = FrontCam('cam0', contours_path_0, cam0_contour_logging)
cam1_solver = SideCam('cam1', contours_path_1, cam1_contour_logging)
cam2_solver = SideCam('cam2', contours_path_2, cam2_contour_logging)
	...
	if door_state == 'DoorOpened':
		...
		if grabResult.GrabSucceeded():
			if cameraContextValue == 0:
				...
				frame2 = cv2.resize(np.uint8(grabResult.Array), (frame_size, frame_size))
			...
			if archive_flag:					
				...
				if cameraContextValue == 0:
					...
					cv2.imwrite('images/cam0/cam0_' + str(time.time()) + '.jpg', frame0)
				elif cameraContextValue == 1:
					writer1.write(serialized)
					cv2.imwrite('images/cam1/cam1_' + str(time.time()) + '.jpg', frame1)
				else:
					writer2.write(serialized)	
					cv2.imwrite('images/cam2/cam2_' + str(time.time()) + '.jpg', frame2)
			...
			if all(check_list):
				...
				#TODO: add id2active_zone* to cv_pick_cam* 
				#Q: how to match active zone to products in cv_pick?
				#does tracking id correspond to order of product?
				...
				if display_mode:
					for img, contours in zip([det_frame0, det_frame1, det_frame2], [contours0, contours1, contours2]):
						draw_contours(img, contours, frame_size)
					
					img_hstack = np.uint8(det_frame0)
					img_hstack = np.hstack((img_hstack, np.uint8(det_frame1)))
					img_hstack = np.hstack((img_hstack, np.uint8(det_frame2)))
					img_hstack = display_fps(img_hstack, fps)
